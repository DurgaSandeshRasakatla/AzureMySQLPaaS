...
resource "azurerm_resource_group" "devopsdbrg" {
  name     = var.rg_name
...
resource "azurerm_mysql_server" "devopsdatabase" {
  name                = var.db_name
  location            = azurerm_resource_group.devopsdbrg.location
  resource_group_name = azurerm_resource_group.devopsdbrg.name
